...
//   BCSC_RESEND_API_KEY      Resend email API key
//   BCSC_WORKSHOP_EMAIL      Workshop notification address
//   BCSC_ZAPIER_WEBHOOK_URL  Zapier webhook for SMS (optional)
//   BCSC_WC_API_KEY          WhatConverts API key
//   BCSC_WC_API_SECRET       WhatConverts API secret
//   BCSC_WC_PROFILE_ID       WhatConverts profile ID (default: 158724)
// ---------------------------------------------------------------------------
...
async function sendEmail(
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Ballarat Commercial Service Centre <onboarding@resend.dev>",
      to: [to],
      subject,
      text,
    }),
...
async function notifyWhatConverts(
  ...
  const landingUrl = new URL("https://service.ballaratcsc.com.au" + (r.landing_page ?? "/"));
...
Deno.serve(async (req: Request) => {
  ...
  const RESEND_API_KEY = Deno.env.get("BCSC_RESEND_API_KEY");
  const WORKSHOP_EMAIL = Deno.env.get("BCSC_WORKSHOP_EMAIL");
  const ZAPIER_WEBHOOK = Deno.env.get("BCSC_ZAPIER_WEBHOOK_URL");
  const WC_API_KEY     = Deno.env.get("BCSC_WC_API_KEY");
  const WC_API_SECRET  = Deno.env.get("BCSC_WC_API_SECRET");
  const WC_PROFILE_ID  = Deno.env.get("BCSC_WC_PROFILE_ID") ?? "158724";
...
Campaign: ${r.utm_campaign ?? ""}`;
    ...
    if (r.customer_email) {
      const customerSubject = `Booking Request Received — Ballarat Commercial Service Centre`;
...
Ballarat Commercial Service Centre
Ballarat, VIC`;
      tasks.push(sendEmail(RESEND_API_KEY, r.customer_email, customerSubject, customerBody));
    }
  } else {
    console.warn("BCSC_RESEND_API_KEY or BCSC_WORKSHOP_EMAIL not set — skipping email");
  }

  if (ZAPIER_WEBHOOK) {
    tasks.push(notifyZapier(r, ZAPIER_WEBHOOK));
  } else {
    console.warn("BCSC_ZAPIER_WEBHOOK_URL not set — skipping Zapier");
  }

  if (WC_API_KEY && WC_API_SECRET) {
    tasks.push(notifyWhatConverts(r, WC_API_KEY, WC_API_SECRET, WC_PROFILE_ID));
  } else {
    console.warn("BCSC_WC_API_KEY / BCSC_WC_API_SECRET not set — skipping WhatConverts");
  }
